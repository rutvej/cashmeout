...
const Timeline = ({ currentDay, totalDays = 7300, financialHealth = 'stable', calendarQueue = null }) => {
  ...
  return (
    <div className="w-full px-3 sm:px-4 pt-3 pb-2 overflow-hidden">
      {/* Date & Age Header Banner */}
      <div className="flex justify-between items-center mb-2 gap-1">
        <div className="flex items-center space-x-1.5 shrink-0">
          <span className="text-[11px] sm:text-xs bg-slate-100 text-slate-700 font-extrabold px-2 py-0.5 rounded-md border border-slate-200 whitespace-nowrap">
            📅 Y{currentYear} · M{monthOfYear}
          </span>
      ...
      <div className="flex items-center justify-between text-[10px] bg-slate-50/90 rounded-xl px-2 sm:px-2.5 py-1 border border-slate-100 gap-1.5 overflow-hidden">
        <div className="flex items-center space-x-1 truncate min-w-0">
          <span className="text-slate-400 font-bold shrink-0">Upcoming:</span>
          {upcomingEvents.length > 0 ? (
            <div className="flex items-center space-x-1 truncate">
              {upcomingEvents.slice(0, 2).map((ue, idx) => (
                <span 
                  key={idx}
                  className="bg-amber-100 text-amber-800 px-1.5 py-0.5 rounded font-bold flex items-center space-x-0.5 border border-amber-200 shrink-0"
